...
// Convert to indexed array and sort by newest date
$salesGrouped = array_values($salesGrouped);
usort($salesGrouped, function ($a, $b) {
      ...
      <table class="table table-bordered table-striped table-hover">
        ...
        <tbody>
          <?php foreach ($salesGrouped as $entry): ?>
            <?php
            $dataGroup = $entry['customer_name'] . '_' . $entry['customer_phone'] . '_' . $entry['created_at'];
            ?>
            <tr>
              ...
              <td><?= htmlspecialchars($entry['gst_type']) ?></td>
              <td>₹<?= number_format($entry['total_price'], 2) ?></td>
              <td><?= date('d-m-Y H:i', strtotime($entry['created_at'])) ?></td>
              <td>
                <button class="btn-generate-invoice"
                  data-group="<?= htmlspecialchars($dataGroup) ?>">
                  Generate Invoice
                </button>
              </td>
            </tr>
...
<script>
  document.querySelectorAll('.btn-generate-invoice').forEach(btn => {
    btn.addEventListener('click', function() {
      ...
      window.open('generate_invoice.php?group=' + encodeURIComponent(groupKey), '_blank', 'width=900,height=700');
    });
  });
</script>